...
```
(node build --copy; node build --composer-install; cd site; vendor/bin/phpunit tests/unit/Espo/Modules/BackupMode)
```
...
```
(node build --copy; node build --composer-install; cd site; vendor/bin/phpunit tests/integration/Espo/Modules/BackupMode)
```
...
If your extension contains additional PHP packages, you also need to add `site/custom/Espo/Modules/BackupMode/vendor` to the *scanDirectories* section in *phpstan.neon* config.
...
* `site/custom/`
* `site/client/custom/`
* `site/tests/unit/Espo/Modules/BackupMode`
* `site/tests/integration/Espo/Modules/BackupMode`
...
2. Set *bundled* and *jsTranspiled* to true in `src/files/custom/Espo/Modules/BackupMode/Resources/module.json`.
3. Add `src/files/custom/Espo/Modules/BackupMode/Resources/metadata/app/client.json`
    ```json
    {
        "scriptList": [
            "__APPEND__",
            "client/custom/modules/backup-mode/lib/init.js"
        ]
    }
    ```
...
Add the library module path to `src/files/custom/Espo/Modules/BackupMode/Resources/metadata/app/jsLibs.json`

```json
{
    "some-lib": {
        "path": "client/custom/modules/backup-mode/lib/some-lib.js"
    }
}
```
